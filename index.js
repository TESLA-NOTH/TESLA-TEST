...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  if (!config.SESSION_ID) return console.log('❌ Please add your session to SESSION_ID config.js !!');

  const sessdata = config.SESSION_ID.replace("NOTH-TESLA~", '');
  const url = `https://tesla-noth-pairing.onrender.com/download/${sessdata}`;
  ...
  https.get(url, (res) => {
    ...
    res.on('end', () => {
      try {
        ...
        console.log("✅ Session connected and saved to creds.json");
      } catch (e) {
        console.error("❌ Invalid JSON received from server:", e.message);
      }
    });
  }).on('error', (err) => {
...
const port = process.env.PORT || 9090;
  ...
  const rayees = ['93744215959', '93730285765', '93794320865'];  // اینجا یک آرایه قرار می‌دهیم
  ...
  app.get("/", (req, res) => {
    res.send(`
      ...
      <head>
        ...
        <title>T E S L A | BOT | STATUS</title>
